...
class Adminuser extends CActiveRecord {
	...
	public function rules() {
		return array (
            //安全设置
            array ('password,username,add_time',
                    'safe'
            ),
            //验证旧密码
            ...
            array ("password","required","message"=>"新密码不能为空",'on'=>'EditPwd,Register'),
            // array ("confirm_pwd","required","message"=>"确认密码不能为空",'on'=>'EditPwd,Register'),
            array ('password', 'length','min'=>6, 'max'=>16,'on'=>'EditPwdRegister'),
            // array ('confirm_pwd', 'length','min'=>6, 'max'=>16,'on'=>'EditPwd,Register'),
            array ("password","validatePassword",'on'=>'EditPwd'),
			//验证图形验证码
            ...
            array ('username','required','message'=>'昵称不能为空','on'=>'Register'),
	...
	public function reg($loginForm,$autoLogin=TRUE) {
		...
		$user_model ['password'] = CPasswordHelper::hashPassword ( $model->password );
		$user_model ['username'] = '';
		$user_model ['add_time'] = time();
	...
	public function getUserByLoginName($loginStr) {
		...
		$criteria = new CDbCriteria ();
		$criteria->addCondition ( 'username=:tel' );
		$criteria->params [':tel'] = $loginName;

		$user = Adminuser::model ()->find ( $criteria );
